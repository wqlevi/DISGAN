"""
A SWinIR take from the following, reconfigured to be 3D compatible
@ author: Qi Wang
"""
...
# Modified again by Zhengliang Wu.
# -----------------------------------------------------------------------------------
...
def window_partition(x, window_size):
    """
    Stack non-overlapped patches into batch-dim
...
class WindowAttention(nn.Module):
    ...
    def __init__(self, dim, window_size, num_heads, qkv_bias=True, qk_scale=None, attn_drop=0., proj_drop=0.):
        ...
        coords = torch.stack(torch.meshgrid([coords_h, coords_w, coords_d]))  # meshgrid with window size
        coords_flatten = torch.flatten(coords, 1)  # 3, Wh*Ww*Wd
        relative_coords = coords_flatten[:, :, None] - coords_flatten[:, None, :]  # pair-wise subtraction
    ...
    def forward(self, x, mask=None):
        """
        Args:
            x: input features with shape of (num_windows*B, N, C), N:flattend window_size
            mask: (0/-inf) mask with shape of (num_windows, Wh*Ww, Wh*Ww) or None
            return: (num_win*B, N, C)
        """
        B_, N, C = x.shape
...
class PatchEmbed(nn.Module):
    ...
    def forward(self, x):
        x = x.flatten(2).transpose(1, 2)  # N C H W -> N Ph*Pw*Pd C
    ...
    def __init__(self, scale, num_feat):
        ...
        if (scale & (scale - 1)) == 0:  # scale = 2^n
            for _ in range(int(math.log(scale, 2))):
                ...
                m.append(PixelShuffle3d(scale=2))
        elif scale == 3:
            m.append(nn.Conv3d(num_feat, 27 * num_feat, 3, 1, 1))
            m.append(PixelShuffle3d(scale=3))
        else:
...
class UpsampleOneStep(nn.Sequential):
    ...
    def __init__(self, scale, num_feat, num_out_ch, input_resolution=None):
        ...
        self.input_resolution = input_resolution
        m = []
        m.append(nn.Conv3d(num_feat, (scale ** 2) * num_out_ch, 3, 1, 1))
