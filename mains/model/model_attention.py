...
class SCPA(nn.Module):
    ...
    def __init__(self, nf, reduction=2, stride=1, dilation=1):
        ...
        self.conv1_a = nn.Conv3d(nf, group_width, kernel_size=1, bias=False)
        self.conv1_b = nn.Conv3d(nf, group_width, kernel_size=1, bias=False)
...
class PAN(nn.Module):
    ...
    def __init__(self, in_nc=1, out_nc=1, nf=64, unf=64, nb=2, scale=2):
        """
        Parameters
        ----------
            in_nc : input channel
            out_nc: output channel
            nf : number of feature per block
            unf : upsample feature
            nb : number of layers per block
        """
        ...
        ### main blocks
        self.SCPA_trunk = PAN.make_layer(SCPA_block_f, nb)
    ...
    def forward(self, x):
        ...
        ILR = F.interpolate(x, scale_factor=self.scale)
        out = out + ILR
        return out
